...
// ─── Mock data ─────────────────────────────────────────────────────────────
// MSL roster sourced from the AAN 2026 congress capture app's TEAM_MEMBERS
// (aurivian-argenx-capture-app) — the more detailed and pre-existing
// source of truth — with VEGA (config/vega.js) realigned to match.

const MSL_OPTIONS = ['Aisha Rahman — National', 'Marcus Brennan — Southeast', 'James Holloway — Midwest', 'Priya Nair — West (gMG)', 'Kevin Park — South (CIDP)', 'Sophie Laurent — Neuromuscular'];

const MSL_PERFORMANCE = [
  { msl: 'Aisha Rahman',  territory: 'National',      status: 'Excellent',     interactions: 52, target: 46, delta: '+6',  qualityScore: 90, kitSignals: 4, kolsAtRisk: 1, note: 'High interaction quality across accounts; strong FcRn competitor differentiation and national coordination.' },
  { msl: 'Marcus Brennan', territory: 'Southeast',    status: 'Quality gap',   interactions: 54, target: 44, delta: '+10', qualityScore: 61, kitSignals: 5, kolsAtRisk: 3, note: 'High volume but insight capture below target — differentiation opportunities not being documented for cross-regional sharing.' },
  { msl: 'James Holloway', territory: 'Midwest',      status: 'Volume gap',    interactions: 39, target: 41, delta: '−2',  qualityScore: 84, kitSignals: 3, kolsAtRisk: 1, note: 'Quality strong but interactions below target — Field Medical Director scope limits pure field time; coverage review needed.' },
  { msl: 'Priya Nair',    territory: 'West (gMG)',     status: 'On track',      interactions: 44, target: 41, delta: '+3',  qualityScore: 81, kitSignals: 4, kolsAtRisk: 2, note: 'Steady gMG-focused engagement; strong SC modality differentiation messaging.' },
  { msl: 'Kevin Park',    territory: 'South (CIDP)',   status: 'Needs support', interactions: 28, target: 39, delta: '−11', qualityScore: 59, kitSignals: 1, kolsAtRisk: 4, note: 'Both volume and quality below target in CIDP-focused territory — escalated to director.' },
  { msl: 'Sophie Laurent', territory: 'Neuromuscular', status: 'On track',      interactions: 36, target: 37, delta: '−1',  qualityScore: 76, kitSignals: 3, kolsAtRisk: 1, note: 'Neuromuscular-focused coverage steady; home self-injection training materials well received.' },
];
...
const HCP_TOPICS = [
  { topic: 'IV→SC Hytrulo conversion readiness',       hcpCount: 55, stages: [10, 15, 18, 9, 3], avgStage: 2.7, trend: '+0.5', confirmed: 24 },
  { topic: 'CIDP earlier-line positioning',             hcpCount: 40, stages: [8, 10, 12, 7, 3],  avgStage: 2.7, trend: '+0.3', confirmed: 16 },
  { topic: 'Home self-injection training confidence',   hcpCount: 31, stages: [6, 9, 10, 4, 2],   avgStage: 2.5, trend: '+0.4', confirmed: 11 },
];
...
const NATIONAL_METRICS = [
  { label: 'Total MSL interactions', value: '253', sub: 'this cycle' },
  { label: 'Avg quality score', value: '75', sub: 'target: 75' },
  { label: 'Insight capture rate', value: '61%', sub: '+2% vs prior' },
  { label: 'KOLs at risk', value: '12', sub: 'tier 1 / tier 2', alert: true },
];

const TERRITORY_LIST = ['National', 'Southeast', 'Midwest', 'West (gMG)', 'South (CIDP)', 'Neuromuscular'];
const TERRITORY_ABBR = ['NAT', 'SE', 'MW', 'gMG', 'CIDP', 'NM'];

// Per-territory HCP topics (scoped subset of national data)
const TERRITORY_HCP_TOPICS = {
  'National': [
    { topic: 'IV→SC Hytrulo conversion readiness', hcpCount: 12, stages: [2, 3, 4, 2, 1], avgStage: 2.7, trend: '+0.5', confirmed: 6 },
    { topic: 'CIDP earlier-line positioning',       hcpCount: 9,  stages: [2, 2, 3, 1, 1], avgStage: 2.6, trend: '+0.3', confirmed: 4 },
    { topic: 'Home self-injection training confidence', hcpCount: 7, stages: [1, 2, 2, 1, 1], avgStage: 2.7, trend: '+0.4', confirmed: 3 },
  ],
  'Southeast': [
    { topic: 'IV→SC Hytrulo conversion readiness', hcpCount: 13, stages: [3, 4, 4, 1, 1], avgStage: 2.3, trend: '+0.3', confirmed: 4 },
    { topic: 'CIDP earlier-line positioning',       hcpCount: 10, stages: [3, 3, 3, 1, 0], avgStage: 2.1, trend: '+0.1', confirmed: 3 },
    { topic: 'Home self-injection training confidence', hcpCount: 6, stages: [2, 2, 1, 1, 0], avgStage: 2.2, trend: '0.0', confirmed: 1 },
  ],
  'Midwest': [
    { topic: 'CIDP earlier-line positioning',       hcpCount: 8,  stages: [1, 2, 3, 1, 1], avgStage: 2.9, trend: '+0.5', confirmed: 4 },
    { topic: 'IV→SC Hytrulo conversion readiness', hcpCount: 7,  stages: [1, 2, 2, 1, 1], avgStage: 2.7, trend: '+0.4', confirmed: 3 },
    { topic: 'Home self-injection training confidence', hcpCount: 5, stages: [1, 1, 2, 1, 0], avgStage: 2.6, trend: '+0.2', confirmed: 2 },
  ],
  'West (gMG)': [
    { topic: 'IV→SC Hytrulo conversion readiness', hcpCount: 11, stages: [2, 3, 4, 1, 1], avgStage: 2.6, trend: '+0.4', confirmed: 5 },
    { topic: 'CIDP earlier-line positioning',       hcpCount: 6,  stages: [1, 2, 2, 1, 0], avgStage: 2.3, trend: '+0.2', confirmed: 2 },
    { topic: 'Home self-injection training confidence', hcpCount: 6, stages: [1, 2, 2, 1, 0], avgStage: 2.5, trend: '+0.3', confirmed: 2 },
  ],
  'South (CIDP)': [
    { topic: 'CIDP earlier-line positioning',       hcpCount: 7,  stages: [2, 2, 2, 1, 0], avgStage: 2.1, trend: '−0.1', confirmed: 2 },
    { topic: 'IV→SC Hytrulo conversion readiness', hcpCount: 5,  stages: [2, 1, 1, 1, 0], avgStage: 2.0, trend: '0.0',  confirmed: 1 },
    { topic: 'Home self-injection training confidence', hcpCount: 4, stages: [2, 1, 1, 0, 0], avgStage: 1.8, trend: '−0.1', confirmed: 0 },
  ],
  'Neuromuscular': [
    { topic: 'Home self-injection training confidence', hcpCount: 9, stages: [2, 2, 3, 1, 1], avgStage: 2.6, trend: '+0.5', confirmed: 3 },
    { topic: 'IV→SC Hytrulo conversion readiness', hcpCount: 7,  stages: [1, 2, 2, 1, 1], avgStage: 2.7, trend: '+0.3', confirmed: 3 },
    { topic: 'CIDP earlier-line positioning',       hcpCount: 5,  stages: [1, 1, 2, 1, 0], avgStage: 2.4, trend: '+0.2', confirmed: 1 },
  ],
};

const TERRITORY_KIT_COVERAGE = {
  'National':       [true,  true,  true,  true,  false],
  'Southeast':      [true,  true,  false, false, false],
  'Midwest':        [true,  true,  false, false, false],
  'West (gMG)':     [true,  false, true,  false, false],
  'South (CIDP)':   [false, true,  false, false, false],
  'Neuromuscular':  [true,  false, true,  false, false],
};

const TERRITORY_BRIEFS = {
  'National': 'National is the strongest-performing territory this cycle — 52 interactions, quality score 90. Aisha Rahman is driving cross-regional FcRn competitor differentiation and national coordination. IV→SC conversion readiness is the standout theme. One tier-1 KOL at risk.',
  'Southeast': 'Southeast shows a quality gap — 54 interactions (above target) but quality score of 61, below the 75 threshold. Marcus Brennan has strong volume but insight capture is inconsistent — differentiation opportunities are not being documented for cross-regional sharing. Three KOLs at engagement risk.',
  'Midwest': 'Midwest is showing a volume gap — 39 interactions against a target of 41. Quality score is strong at 84. James Holloway\'s Field Medical Director scope limits pure field time; CIDP earlier-line positioning is the strongest awareness topic here.',
  'West (gMG)': 'West (gMG) is on track with 44 interactions and quality score 81. Priya Nair is delivering steady gMG-focused engagement with strong SC modality differentiation messaging. Two KOLs at risk.',
  'South (CIDP)': 'South (CIDP) requires immediate support — 28 interactions (72% of target) and quality score 59. Kevin Park has 4 tier-1/2 KOLs with no substantive contact in the past 60 days. Escalated to field director.',
  'Neuromuscular': 'Neuromuscular is on track with 36 interactions and quality score 76. Sophie Laurent\'s home self-injection training materials are being well received — the highest-confidence topic in her territory. One KOL at risk.',
};

const MO_FIELD_DATA = {
  MO1: { interactions: 58, insights: 2, kols: 8, impact: 'IV→SC conversion narrative in development; home self-injection toolkit deployed to top accounts.' },
  MO2: { interactions: 41, insights: 1, kols: 5, impact: 'ADHERE post-hoc sub-analysis commissioned; MSL briefing not yet deployed.' },
  MO3: { interactions: 17, insights: 1, kols: 3, impact: 'Tier-1 pipeline KOL outreach plan drafted (6 candidates); no owner assigned yet.' },
  MO4: { interactions: 22, insights: 1, kols: 4, impact: 'Long-term registry RWE package scoped but not yet built for congress or field use.' },
  MO5: { interactions: 12, insights: 0, kols: 2, impact: 'No active insight this cycle; access barriers currently well-managed relative to target.' },
};
...
function FieldBrief({ scope }) {
  const briefs = {
    national: 'National field performance is tracking at 75/100 quality score, on target. IV→SC Hytrulo conversion (MO1) shows the strongest field signal activity — 58 interactions and 2 insights captured this cycle. KOL engagement risk is elevated in South (CIDP): 4 tier-1 KOLs have had no substantive contact in the past 60 days. Pipeline KOL engagement (MO3) continues to generate low field interaction volumes — it\'s primarily a NOVA analysis-engine signal, not yet an MSL activity — directed outreach is recommended.',
      ...
      <section>
        ...
        <div className="space-y-2">
          {LISTENING_PRIORITIES.map((lp) => {
            ...
            return (
              <div key={lp.id} className="rounded-xl border border-auri-border bg-auri-card p-4">
